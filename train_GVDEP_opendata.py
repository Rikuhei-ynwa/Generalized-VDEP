...
if args.skip_load_rawdata:
    ...
    if args.data == "statsbomb":
        ...
        data_folder = f"../open-data-{args.date}/data"  # Example of local folder with statsbomb data
...
if args.skip_train:
    ...
    def getXY(games: pd.DataFrame, Xcols: list, nan_count=False):
        """
        Parameters
        ...
        if "dist_at0_a0" in X.columns.values and "dist_df0_a0" in X.columns.values:
            if nan_count:
                nan_array = np.empty(len(X))
                for i in range(len(X)):
                    off_ball_features = X.loc[i,'at0_x_a0':]
                    count_nan = int(np.count_nonzero(np.isnan(np.asarray(off_ball_features).astype(float))))
                    nan_array[i] = count_nan
                nan_array = nan_array / 4
    ...
    X, Y, drop_index = getXY(traingames, Xcols, nan_count=True)
    trainX_vaep, _, _ = getXY(traingames, Xcols_vaep, nan_count=False)
    print("X:", list(X.columns), ", length: ", str(len(X.columns)))
    print("Y:", list(Y.columns))

    testX, testY, _ = getXY(testgames, Xcols, nan_count=False)
    testX_vaep, _, _ = getXY(testgames, Xcols_vaep, nan_count=False)
...
if args.test:
    ...
    sorted_A = sorted_A[
        ...
        # & ((sorted_A.type_name.str.contains("tackle")) | (sorted_A.type_name.str.contains("interception")) | (sorted_A.type_name.str.contains("clearance")))
        & (sorted_A.type_name.str.contains("clearance"))
    ]
...
# team_values_df = (
#     A[["defense_team", "gains", "effective_attack", "vdep_value"]].groupby(["defense_team"]).mean().reset_index()
# )

# result_df = team_values_df[team_values_df["defense_team"].isin(knockout_teams_list).values].reset_index(drop=True)
# result_df = pd.merge(result_df, concedes_ser, left_on="defense_team", right_index=True)
# result_df = result_df.reindex(
#     columns=["defense_team","concedes","gains","effective_attack","vdep_value",]
#     )
...
for v in itertools.combinations(result_df.columns.values.tolist()[1:], 2):
    ...
    # Plot a figure
    fig_team = plt.figure(figsize=(12, 9))

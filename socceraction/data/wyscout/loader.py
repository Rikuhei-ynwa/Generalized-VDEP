...
from ..base import (EventDataLoader, MissingDataError, ParseError,
                    _localloadjson, _remoteloadjson)
from .schema import (WyscoutCompetitionSchema, WyscoutEventSchema,
                     WyscoutGameSchema, WyscoutPlayerSchema, WyscoutTeamSchema)
...
class WyscoutLoader(EventDataLoader):
    ...
    def competitions(self) -> DataFrame[WyscoutCompetitionSchema]:
        ...
        if 'competitions' in self.feeds:
            ...
            obj = self.get(path)
            # obj = self.get(path)[0] # changed
            if False: # not isinstance(obj, dict): #  or 'competitions' not in obj: # changed
                raise ParseError('{} should contain a list of competitions'.format(path))
            seasons_urls = [self._get_file_or_url('seasons', competition_id=c['wyId'])[0] for c in obj]
            #seasons_urls = [ # changed

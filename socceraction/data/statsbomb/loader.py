...
from .schema import (StatsBombCompetitionSchema, StatsBombEventSchema,
                     StatsBombGameSchema, StatsBombPlayerSchema,
                     StatsBombTeamSchema)
...
class StatsBombLoader(EventDataLoader):
    ...
    def events(self, game_id: int, load_360: bool = False) -> DataFrame[StatsBombEventSchema]:
        ...
        if self._local:
            try: obj = _localloadjson(str(os.path.join(self._root, f"three-sixty/{game_id}.json")))
            except:
